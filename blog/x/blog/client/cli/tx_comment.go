...
func CmdCreateComment() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			argsBody, err := cast.ToStringE(args[0])
			if err != nil {
				return err
			}
			...
			msg := types.NewMsgCreateComment(clientCtx.GetFromAddress().String(), argsBody, argsPostID)
...
func CmdUpdateComment() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			argsPostID, err := cast.ToStringE(args[2])
			if err != nil {
				return err
			}
			...
			msg := types.NewMsgUpdateComment(clientCtx.GetFromAddress().String(), id, argsBody, argsPostID)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
			return tx.GenerateOrBroadcastTxCLI(clientCtx, cmd.Flags(), msg)
